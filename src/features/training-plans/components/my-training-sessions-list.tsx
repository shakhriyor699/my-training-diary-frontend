...
type MyTrainingSessionsListProps = {
  locale: string;
...
export function MyTrainingSessionsList({
  locale,
  response,
  query,
  labels,
}: MyTrainingSessionsListProps) {
          ...
          {response.data.map((session) => {
            const totalVolume = session.exerciseLogs.reduce(
              (sum, exerciseLog) =>
                ...
                exerciseLog.sets.reduce(
                  (exerciseSum, setLog) => exerciseSum + setLog.weight * setLog.reps,
                  0,
                ),
              0,
            );
            ...
            return (
              <Link
                ...
                className="block rounded-[20px] border border-white/8 bg-black/20 p-5 transition-colors hover:border-white/16 hover:bg-black/28"
              >
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="grid min-w-[260px] gap-3 rounded-[18px] border border-white/8 bg-white/[0.03] p-4 text-sm text-white/64">
                    ...
                    <Metric
                      ...
                      value={formatInteger(totalSets)}
                    />
                    <Metric
                      label={labels.totalVolume}
                      value={formatInteger(totalVolume)}
                    />
                  </div>
                </div>
...
function PaginationLink({
  ...
  const href = `/${locale}/dashboard/sessions?${new URLSearchParams({
