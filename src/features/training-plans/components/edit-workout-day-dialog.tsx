...
import { updateWorkoutDay } from "@/src/features/training-plans/api/update-workout-day";
...
export function EditWorkoutDayDialog({
  ...
  const mutation = useMutation({
    mutationFn: (payload: CreateWorkoutDayInput) =>
      updateWorkoutDay(day.id, payload),
    onSuccess: () => {
      startTransition(() => {
        setIsOpen(false);
        setFormError(null);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <div className="w-full max-w-xl rounded-[24px] border border-white/8 bg-[#090909] p-6 shadow-[0_24px_120px_rgba(0,0,0,0.42)]">
            ...
            <form className="space-y-4" onSubmit={onSubmit}>
              <Field label={labels.titleLabel} htmlFor={`edit-day-title-${day.id}`}>
                <input
                  id={`edit-day-title-${day.id}`}
                  type="text"
              ...
              <Field label={labels.orderLabel} htmlFor={`edit-day-order-${day.id}`}>
                <input
                  id={`edit-day-order-${day.id}`}
                  type="number"
                  min="1"
