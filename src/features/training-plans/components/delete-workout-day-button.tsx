...
import { deleteWorkoutDay } from "@/src/features/training-plans/api/delete-workout-day";
import type { TrainingPlanWorkoutDay } from "@/src/features/training-plans/lib/training-plans.types";
import { cn } from "@/src/shared/lib/utils";

type DeleteWorkoutDayButtonProps = {
...
export function DeleteWorkoutDayButton({
  day,
  triggerClassName,
  labels,
}: DeleteWorkoutDayButtonProps) {
  const router = useRouter();
  const [isOpen, setIsOpen] = useState(false);
  const [isPending, startTransition] = useTransition();

  const mutation = useMutation({
    mutationFn: () => deleteWorkoutDay(day.id),
    onSuccess: () => {
      startTransition(() => {
        setIsOpen(false);
  ...
  return (
    <>
      <button
        ...
        className={cn(
          "inline-flex h-10 items-center justify-center rounded-xl border border-[#ff6b5d]/20 bg-[#ff6b5d]/10 px-4 text-sm font-medium text-[#ff9b90] transition-colors hover:bg-[#ff6b5d]/16",
          triggerClassName,
        )}
      >
        {labels.trigger}
