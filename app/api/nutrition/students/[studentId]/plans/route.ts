...
import { createStudentNutritionPlanSchema } from "@/src/features/nutrition/lib/create-student-nutrition-plan.schema";
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const upstreamResponse = await fetch(
    `${env.API_BASE_URL}/nutrition/students/${parsedStudentId}/plans`,
    {
      method: "GET",
      headers: {
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const parsedBody = createStudentNutritionPlanSchema.safeParse(json);
    ...
    return NextResponse.json(
      {
        message:
          (responseBody &&
            ...
            typeof responseBody.message === "string" &&
            responseBody.message) ||
          "Unable to create nutrition plan for student.",
      },
      { status: upstreamResponse.status },
    );
  }
